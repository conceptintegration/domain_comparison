...
def process(config):
    # Encoder
    encoder = hub.load(config['encoder']['path'] + config['encoder']['model'])

    print('Loading segment encodings…')
    with open(config['model_path'] + 'segment_encodings.json', 'r', encoding='utf-8') as f:
        segment_encodings = json.load(f)
        f.close() 

    ontology_dict = {}
    for file in config['ontology_files']:
        ont_label = file.split('/')[0]
        ontology_dict[ont_label] = {}

        with open(config['data_path'] + file, encoding='utf-8', errors='replace') as f:
            reader = csv.reader(f)
            # Get the header row
    ...
    for ont_label,ont_data in ontology_dict.items():
        ...
        filename = config['model_path'] + ont_label + '_topics_dict.json'
        with open(filename, 'w') as f:
            json.dump(topics_dict, f)
            f.close()
        topic_encodings = encode_topics(ont_label,topics_dict,config,encoder,split_size=2)
        build_topic_segments_matrix(ont_label,topic_encodings,segment_encodings,config)
        print()
